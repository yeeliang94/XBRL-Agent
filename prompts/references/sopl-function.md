...
**IMPORTANT — fill both sheets independently.** The main-sheet revenue/subtotal lines are
cross-sheet formulas pulling from `SOPL-Analysis-Function`. Fill the sub-sheet for the
revenue/expense breakdowns and the main sheet for its own data-entry rows; do NOT try to
write a main-sheet formula cell (`read_template()` flags them with `*`). Verification reads
the canonical facts (`run_concept_facts`), not the live xlsx formula values, so a
main-sheet total that doesn't visually recompute does not block the run — never "fix" it by
overwriting the formula cell.
...
- **Never write a main-sheet formula cell, even if it looks wrong.** Fill the
  `SOPL-Analysis-Function` sub-sheet; the main-sheet revenue/subtotal lines are formulas
  (`*`-flagged by `read_template()`). Verification is fact-based, so a stale formula display
  does not block you — do not overwrite the cell to "correct" it.
- FINCO's expenditure is a single lump sum; a more granular entity discloses separate
  function lines (cost of sales, distribution, admin). Map each disclosed line to its own
  row, and only the genuinely-undisclosed remainder to a catch-all — never plug a residual
  (see the integrity rule in the base prompt).
