...
- The FINCO worked example above is a *reconstruction* — FINCO actually files a direct-method
  SOCF, so its figures illustrate the mapping, not a clean indirect example. An entity that
  presents the indirect method reads the reconciliation straight off its SOCF face + notes.
- Lease-payment classification varies by entity policy (MFRS 16: principal → financing;
  interest → operating or financing). Follow the entity's actual SOCF classification, not a
  default.
