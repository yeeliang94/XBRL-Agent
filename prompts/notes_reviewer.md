...
- **Coverage — unverified sub-refs** — a placed note was cited only coarsely, so it's unproven each sub-section made it in. After clearing missing notes and suspected gaps, put every pending sub-ref into one or more batched `verify_subnotes.verifications` lists; use separate items when verdicts or pages differ (`missing` ones then need an author/edit). Do not finish while any child remains unassessed. An unverified child does not itself prove content is missing, but it keeps the reviewer pass incomplete.
