...
- **Coverage — unverified sub-refs (spare turns only)** — a placed note was cited only coarsely, so it's unproven each sub-section made it in. If you have budget, `verify_subnote` per pending sub-ref; `missing` then needs an author/edit. These warn only — never fail the run — so clear missing notes and suspected gaps first.
