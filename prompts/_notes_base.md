...
prompt below says whether such a row exists.

For the List of Notes sheet, **one top-level PDF note goes to one template
field**. Copy the full note into that one field. Do not distribute its
sub-sections, paragraphs, tables, or line items across different List-of-Notes
rows. The only content removed from that field is an explicitly labelled
Material/Significant Accounting Policy section routed to the Accounting
Policies sheet under the carve-out rule below.
...
with 5.1 and 5.2 in one cell). Do not feed one disclosure note into multiple
...
For the List of Notes sheet, the top-level heading controls the single target
field:

- A top-level numbered note maps to one List-of-Notes field. If
...
- Do NOT split content into a different template row merely because the auditor used
  "(a)", "(b)", "(i)", "(ii)", bullets, table captions, materially different
...
- A sub-section explaining how a parent balance is measured, depreciated,
  ...
  Keep it with the parent disclosure even when the PDF gives it its own
  sub-number or the template has another row that mentions its topic.
  ...
  depreciation row even if one exists.
- Choose the row that best represents the top-level note's printed heading
  and primary subject. A topic merely discussed inside the note never gets a
  separate row. If no specific row can represent the full note, put the full
  note in the List-of-Notes catch-all field. Never break it apart to make
  individual pieces fit narrower rows.
- Before writing, confirm that every paragraph, sub-section and table from the
  note is present in that one payload, except for an explicitly labelled
  accounting-policy carve-out. Nothing else is omitted, duplicated, or sent
  to another List-of-Notes field.
