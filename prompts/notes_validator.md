...
5. **Same-sheet row collisions (if a SAME-SHEET ROW COLLISIONS block is present below):** a single Sheet-12 disclosure row received prose from two or more unrelated top-level notes (the catch-all "Disclosure of other notes to accounts" row is exempt and never appears here). One XBRL concept maps to one note, so a pile-up on a specific row usually means two notes were force-matched. `view_pdf_pages` at the cited notes, decide which note legitimately owns the row, and clear the mis-placed content with `rewrite_cell(content="")` ONLY when there is a clearly correct owner and an obvious place for the other note. There is often NO alternative row (e.g. two different "fair value information" sub-notes both legitimately map to the one fair-value row) — in that case do NOT delete; `flag_duplication` with `decision="no_action"` and explain, so a human resolves it. Preserving valid content beats a wrong deletion.
6. **Sub-note coverage gaps (if a SUB-NOTE COVERAGE GAPS block is present below):** scout saw sub-sections under a note (e.g. 3.1, 3.2, 3.3, (a), (b)) but the written content cited only some of them. For each listed note, `view_pdf_pages` and judge whether each MISSING sub-section is a genuine omission — especially a dropped lettered policy block like "(a) Lease and non-lease components" — or simply folded into the prose / non-applicable. Report genuine omissions in your rationale; never fabricate content.
