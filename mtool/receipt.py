...
* **A receipt is never load-bearing for the fill.** A write failure is logged
  and swallowed — losing the audit trail is bad, failing an otherwise-good
  fill because the audit write hiccuped is worse.
"""
...
def write_fill_receipt(
    ...
    *,
    ...
    """Insert one receipt; return its id (``None`` if the write failed).

    Never raises — see the module docstring on why the audit trail must not be
    able to fail a good fill.
    """
    try:
        conn = sqlite3.connect(str(db_path))
        try:
        ...
        logger.warning("mTool fill receipt write failed for run %s",
