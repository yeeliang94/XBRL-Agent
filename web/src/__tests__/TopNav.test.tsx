...
describe("TopNav", () => {
  test("renders New extraction + Runs for everyone; Field labels only for admins", () => {
    ...
    expect(screen.getByRole("tab", { name: /new extraction/i })).toBeTruthy();
    expect(screen.getByRole("tab", { name: /runs/i })).toBeTruthy();
  ...
  test("active view has aria-selected=true", () => {
    const { rerender } = render(<TopNav view="extract" onViewChange={() => {}} />);
    expect(
      screen.getByRole("tab", { name: /new extraction/i }).getAttribute("aria-selected"),
    ).toBe("true");
    expect(
      screen.getByRole("tab", { name: /runs/i }).getAttribute("aria-selected"),
    ).toBe("false");
  ...
  test("clicking a button fires onViewChange with that view", () => {
    ...
    fireEvent.click(screen.getByRole("tab", { name: /new extraction/i }));
  ...
  test("active button has a visually distinct style vs inactive", () => {
    ...
    const activeBtn = screen.getByRole("tab", { name: /runs/i }) as HTMLButtonElement;
    const inactiveBtn = screen.getByRole("tab", { name: /new extraction/i }) as HTMLButtonElement;
