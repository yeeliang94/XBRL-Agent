...
# ---------------------------------------------------------------------------
# Doc conversions (scanned-PDF → readable-document feature, schema v21).
# Standalone job records, independent of the extraction pipeline. Durable like
# run_review_tasks: a finished conversion survives a restart, and a stale
# 'running' row left by a crash is reconciled at startup.
# ---------------------------------------------------------------------------

@dataclass
class DocConversion:
    """One scanned-PDF → readable-document conversion job."""
    ...
    total_pages: int
    current_page: int
    result_html_path: Optional[str]
    error: Optional[str]
    created_at: str
    updated_at: str


def _row_to_doc_conversion(row: sqlite3.Row) -> DocConversion:
...
def create_doc_conversion(
    conn: sqlite3.Connection,
    *,
    source_pdf_path: str,
    original_filename: str,
) -> int:
    """Insert a new conversion job in the 'queued' state; return its id."""
    now = _now()
    cur = conn.execute(
        "INSERT INTO doc_conversions(source_pdf_path, original_filename, "
        "status, total_pages, current_page, created_at, updated_at) "
        "VALUES (?, ?, 'queued', 0, 0, ?, ?)",
        (source_pdf_path, original_filename, now, now),
    )
    return int(cur.lastrowid)


def create_doc_conversion_if_idle(
    db_path: str | Path,
    *,
    source_pdf_path: str,
    original_filename: str,
) -> Optional[int]:
    """Atomically create a conversion job IFF none is active; else return None.

    "One conversion at a time" (PRD v1) implemented as an atomic check-and-insert
    under `BEGIN IMMEDIATE`: the immediate write-lock means two concurrent
    callers can't both pass the idle check and create duplicate jobs (robust even
    under multiple workers, not just the current single async worker). Opens its
    own connection so the lock scope is exactly the check+insert.
    """
    conn = sqlite3.connect(str(db_path))
    conn.row_factory = sqlite3.Row
    conn.execute("PRAGMA busy_timeout = 5000")
    try:
        # IMMEDIATE acquires the reserved lock now (not lazily at first write),
        # so a second concurrent caller blocks here until we commit/rollback.
        conn.execute("BEGIN IMMEDIATE")
        active = conn.execute(
            "SELECT 1 FROM doc_conversions WHERE status IN ('running', 'queued') "
            "LIMIT 1"
        ).fetchone()
...
def fetch_doc_conversion(
    conn: sqlite3.Connection, job_id: int
) -> Optional[DocConversion]:
    """Return one conversion job, or None if it doesn't exist."""
    prior_factory = conn.row_factory
    conn.row_factory = sqlite3.Row
    try:
        row = conn.execute(
            "SELECT * FROM doc_conversions WHERE id = ?", (job_id,)
        ).fetchone()
    finally:
        conn.row_factory = prior_factory
    return _row_to_doc_conversion(row) if row is not None else None


def update_doc_conversion_progress(
    conn: sqlite3.Connection,
    job_id: int,
    *,
    current_page: int,
    ...
    """Update page progress (and optionally flip status to 'running')."""
    conn.execute(
        "UPDATE doc_conversions SET status = ?, current_page = ?, "
        "total_pages = ?, updated_at = ? WHERE id = ?",
        (status, current_page, total_pages, _now(), job_id),
    )


def mark_doc_conversion_finished(
    conn: sqlite3.Connection,
    job_id: int,
    *,
    status: str,
    ...
    """Land a job in a terminal state ('done' or 'failed').

    On success pass `result_html_path`; on failure pass `error`. Either way
    the job leaves 'running' so a poll resolves (terminal-status contract,
    gotcha #10).
    """
    conn.execute(
        "UPDATE doc_conversions SET status = ?, result_html_path = ?, "
        "error = ?, updated_at = ? WHERE id = ?",
        (status, result_html_path, error, _now(), job_id),
    )


def is_doc_conversion_running(conn: sqlite3.Connection) -> bool:
    """True if any conversion is currently 'running' or 'queued'.

    Used to serialise conversions to one at a time (PRD v1 decision).
    """
    row = conn.execute(
        "SELECT 1 FROM doc_conversions WHERE status IN ('running', 'queued') "
        "LIMIT 1"
...
def reconcile_stale_doc_conversions(conn: sqlite3.Connection) -> int:
    """Retire conversions orphaned by a process restart; return rows fixed.

    A conversion runs on a daemon thread that dies with the process. A row left
    'queued'/'running' after a restart can never finish, so flip it to a
    terminal 'failed' with an honest message (mirrors
    reconcile_stale_review_tasks). Called once at startup.
    """
    cur = conn.execute(
        "UPDATE doc_conversions SET status = 'failed', "
        "error = 'Server restarted while the conversion was running. "
        "Please convert again.', updated_at = ? "
