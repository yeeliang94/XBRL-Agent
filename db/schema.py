...
# v38 adds `mtool_fill_receipts` — the durable record of every mTool fill
# (docs/PLAN-mtool-fill-pipeline.md Step 19; originally minted as v35 by the
# reverted b04b178, re-minted here as v38 because v35–v37 were taken by the
# notes source-integrity model in the meantime). A filled MBRS workbook is a
...
_CREATE_STATEMENTS: tuple[str, ...] = (
    ...
    """,
    # -----------------------------------------------------------------
    # v38: mtool_fill_receipts — durable record of every mTool fill
    # (docs/PLAN-mtool-fill-pipeline.md Step 19; re-minted from the reverted
    # v2 build's v35). The patcher itself stays stateless and stdlib-only;
    # the ROUTE writes this row. `snapshot_*` identifies the exact fact
    # revision the workbook was built from, so two fills of the same
    # still-editable run are distinguishable. No CHECK on `status`
    # (gotcha #11) and every column nullable/defaulted.
    # -----------------------------------------------------------------
    """
...
def init_db(path: str | Path) -> None:
    ...
    try:
        ...
        # v37 → v38: add mtool_fill_receipts (durable mTool fill record). Pure
        # CREATE TABLE IF NOT EXISTS (already run above), so this block only
        # advances the marker. Same BEGIN IMMEDIATE + re-check discipline.
        if current_version is not None and current_version < 38:
            try:
                conn.execute("BEGIN IMMEDIATE")
                row = conn.execute(
                    "SELECT version FROM schema_version LIMIT 1"
                ...
                latest = int(row[0]) if row else None
                if latest is not None and latest < 38:
                    conn.execute(
                        "UPDATE schema_version SET version = ?",
                        (38,),
                    )
