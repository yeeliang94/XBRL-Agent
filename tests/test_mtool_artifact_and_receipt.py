...
def client(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("XBRL_MTOOL_FILL", "1")
...
def test_the_route_adds_no_guard_of_its_own_around_the_receipt(client, tmp_path,
                                                               monkeypatch):
    """Where the protection lives, pinned so it can't silently move.

    The route calls ``write_fill_receipt`` bare. That is safe ONLY because the
    function swallows its own errors (next test) — if someone ever replaces it
    with something that raises, the fill goes down with it. This test makes
    that dependency explicit rather than leaving it to a comment; it also
    proves the temp dir is still cleaned up when the fill dies late.
    """
    tc, db, _ = client
    import api.mtool as m
...
def test_receipt_module_swallows_its_own_write_errors(tmp_path):
    from mtool.receipt import write_fill_receipt
    missing = tmp_path / "no-such-dir" / "x.db"
    assert write_fill_receipt(
        ...
        operator=None, report=None) is None
