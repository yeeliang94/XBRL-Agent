...
**Overall Progress:** `100%` _(local + Mac smoke; Windows live smoke still owed)_
...
| Step | Status | Description |
|------|--------|-------------|
| 4.1 | 🟩 | Mac smoke (2026-05-21): drove `read_template()` + `_summarize_template()` directly on 3 MPERS templates. Group `01-SOFP-CuNonCu`: 59 `[ABSTRACT]` lines; Group `03-SOPL-Function`: 23 (including the Bug A failure-mode rows `Other fee and commission income`, `Other expenses`, `Interest income`); Group `09-SOCIE`: 24 (all 4 block headers + per-block sub-headers). |
