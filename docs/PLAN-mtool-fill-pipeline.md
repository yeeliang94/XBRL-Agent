...
finding is now closed in code and pinned by tests, and the action is gated OFF
until it has been proven on Windows. **Nothing here is filing-ready yet**, and
that is now a deliberate, enforced state rather than an oversight:
`XBRL_MTOOL_FILL` defaults off, so no filing-capable action is exposed while the
two Windows-blocked questions (Steps 1 and 7) remain open.
