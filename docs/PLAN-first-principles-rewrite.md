...
**Overall Progress:** ✅ **MERGED TO `main`** (2026-05-31) — `main` and `rewrite/first-principles` are level at `3b72e56`; all 40 rewrite commits are in `main`'s history above `pre-rewrite-baseline` (`80e20c7`). The deliberate stopping points below (Step 2.3 Sheet-12 fan-out, Step 5.2 full Extract/drain rewrite, Step 6.2 `system_event` envelope collapse) landed as-decided — they are documented exceptions, not pending work. Original status retained below for the audit trail.

**Overall Progress (pre-merge):** `~95%` — Phases 0, 1, 2, 3, 4 COMPLETE (Step 4.3 fixed Group date headers via scout-metadata synthesis, 2026-05-30); Phases 5.1, 5.3, 6.1, 6.2, 6.3 COMPLETE (Step 5.1 split all 28 routes out of `server.py` into `api/`, 2026-05-30; branch green: backend 2 failed [pre-existing doc invariants] / 1850 passed / 2 skipped; frontend 631 passed). **Step 5.2 PARTIAL** (2026-05-30, user-chose surgical phase-units): the Validate phase is extracted into a standalone structured-result unit (`_validate_and_build_run`); the Extract/drain core stays in the generator shell because factoring a `yield` out would break the GeneratorExit-tolerant drain (gotcha #19) — deferred to a dedicated session along with PR-1 (fold `run.py` into the shared pipeline). Remaining: **5.2 full Extract/drain rewrite + PR-1** (no live-LLM needed). Phase 4 (store-first keystone) commit `c61afeb` — render-last + render-from-facts; render-layer A/B clean on **FINCO Company** (5 date-cell diffs, 0 regressions; self-diff 0). **Peer review 2026-05-30 found Phase 4 was prematurely marked DONE: Group reporting-period date headers can't reach the download (writer refuses row-2 labelless writes; prompt only writes row 1), and the date carry-forward still depends on the scratch workbook. Step 4.3 has the user-approved fix (synthesize dates from Infopack metadata).** Phase 6.2 (3-bucket error taxonomy + `bucket` field on every coordinator SSE error; frontend drives `isRunning` off it) shipped 2026-05-30. Phase 6.1 (precomputed `concept_targets`, single exporter lookup) shipped 2026-05-30. Phase 5.3 (durable `run_review_tasks` table, schema v13) shipped 2026-05-30. Phase 3 (typed `write_facts`) shipped 2026-05-30. Once 5.1 + 5.2 land, the rewrite is ready for the merge gate to `main`. *(Update 2026-05-31: this gate has been passed — the branch is merged to `main`; see the MERGED banner above.)*
**PRD Reference:** [docs/REWRITE-first-principles.html](REWRITE-first-principles.html)
**Last Updated:** 2026-05-31 (merged to `main`)
**Branch:** `rewrite/first-principles` — **merged to `main`** at `3b72e56` (off `main`; baseline tag `pre-rewrite-baseline`)
...
- [x] 🟩 **Step 5.1: Split `server.py` routes into `api/`** — DONE (2026-05-30). All 28 FastAPI route handlers moved out of `server.py` (6269 → 4286 LOC) into seven cohesive routers under `api/`: `config_routes.py` (config/settings/test-connection), `uploads.py` (upload/scout), `run_control.py` (run/start/abort/rerun/rerun-notes), `reviewer.py` (re-review/status/revert), `runs.py` (list/detail/trace/patch/delete/recheck), `notes.py` (notes_cells GET+PATCH/edited_count/facts/edited_count), `files.py` (pdf info/page, download/filled, result download). `server.py` keeps app wiring, lifespan, shared state, the helpers, and `run_multi_agent_stream` (Step 5.2 turns that into the phase pipeline), plus the date-range middleware + SPA mount (both `@app.`-level, can't live on an `APIRouter`).
  - **Design: `server.X` stays the namespace of record.** Each router does `import server` and reads shared state/helpers/models as `server.X` *at call time* (lazy in-body project imports come with each handler, so no import cycle). This preserves the existing test monkeypatch surface verbatim — `patch("server._create_proxy_model")`, `setattr(server, "OUTPUT_DIR", …)`, `patch("server.run_multi_agent_stream")` all keep working because the live handler reads `server.X`. The `lambda: AUDIT_DB_PATH` closure already in use for the concept routers is the same idiom. Routers are imported + `include_router`'d at the BOTTOM of `server.py` (after every shared symbol is defined) and BEFORE the SPA catch-all. The moved handler functions are also re-exported as `server.<name>` so the two tests that invoke a handler directly (`server.download_result(...)`) still resolve.
  - **Helpers that stay in `server.py`** (referenced as `server.X`): `_open_audit_conn`, `_run_summary_to_dict`, `_resolve_api_key`, `_load_*`, `_export_canonical_workbooks`, `_run_has_facts`, `_reexport_*`, `_recheck_from_facts`, `_refresh_persisted_cross_checks`, `_safe_downgrade_run_status`, `_create_proxy_model`, `_run_reviewer_pass`, `_active_flag_guidance`, `_save_review_task`, `_reexport_remerge_durable`, and the `RunConfigRequest`/`RunConfigPatchRequest` models. The file-route-local helpers (`_resolve_run_pdf_path`, `_remove_overlay_tempfiles`, the DPI clamp + download whitelist) moved INTO `api/files.py` since nothing else uses them.
  - **Verify:** ✅ full backend **2 failed (pre-existing `test_docs_invariants`) / 1850 passed** (= baseline); frontend **631 passed**. `tests/test_server_run_lifecycle.py`, `test_reviewer_routes.py`, `test_runs_start_endpoint.py`, `test_legacy_run_endpoint.py`, `test_download_reexport.py`, `test_download_api.py`, `test_stop_all_preserves_partial.py`, `test_pipeline_stage_events.py` all green. Zero behaviour change — pure route relocation.

- [~] 🟨 **Step 5.2: Make the run pipeline explicit (surgical phase-units)** — PARTIAL (2026-05-30, user-chosen low-risk approach). The **Validate** phase is now a standalone unit (`server._validate_and_build_run`) returning ONE structured result: either a populated `_ValidatedRun` (statements/variants/models/notes/infopack/model/config) or `(None, fail_events, fail_status)` carrying the SSE error events the generator yields + the terminal status it records. Every failure path still routes through `_fail_run` exactly as before — the only change is the events are *returned* rather than yielded in place, so the ~165-line block lives outside the generator without touching the lifecycle contract. The generator's docstring now enumerates the explicit phase sequence (Validate → Extract → Cascade → Merge/Render → Check → Review → Persist/Finalize), and the boundaries remain marked by the existing `_emit_stage(...)` events.
  - **Why the rest stays in the generator shell (deliberate, not unfinished):** the user chose "surgical phase-units (recommended)" over a full structured-result rewrite. The Extract/Cascade/Check/Review/Persist stages are each interleaved with the **GeneratorExit-tolerant `event_queue` drain** (gotcha #19) — concurrent coordinator/notes tasks push events while the generator `yield`s them and flips `client_connected` on disconnect. Factoring a `yield` out of an async generator into a returning unit would break that disconnect-finalization contract and the terminal-status `finally` (gotcha #10). The post-extraction *work* is already delegated to named helpers (`_run_reviewer_pass`, `merge_workbooks`, `run_cross_checks`, `_export_canonical_workbooks`, `_attempt_partial_merge`, `_emit_cross_check_summary`), so the generator is already the thin orchestration shell; Validate was the one large block that was pure + yield-free and therefore safe to lift.
  ...
  - [x] 🟩 **PR-1 DONE (2026-05-30):** `run.py::run_agent` now drives the SAME `server.run_multi_agent_stream` pipeline as the web UI instead of building a bare `RunConfig` + scratch merge. It copies the source PDF to `session_dir/uploaded.pdf`, runs the canonical concept-tree bootstrap the CLI previously skipped (the server does it in its lifespan; without it fact projection can't resolve concepts), builds a `RunConfigRequest`, and async-drives the generator — so a CLI run now creates an audit `runs` row, threads `run_id`/`db_path` into the coordinator (fact projection), exports from facts, runs cross-checks, and (when auto-review is on + checks fail) reviews. The CLI-only helpers `_next_run_dir` / `build_parser` / `AgentResult` are unchanged. Pinned by `tests/test_cli_canonical_pipeline.py` (mocked pipeline: asserts the coordinator receives `run_id`+`db_path` and the audit DB lands a `completed` run + per-agent rows); `test_run_numbering` + `test_mpers_wiring` (parser) still green.
  - [ ] 🟦 **Deliberately NOT done (deferred, with rationale):** the full `PHASES = [...]` structured-result rewrite of the **Extract/drain core**. After reading the whole generator, this is judged high-risk / low-value: the Extract/Cascade/Check/Review/Persist stages are each interleaved with the GeneratorExit-tolerant `event_queue` drain (gotcha #19) + terminal-status `finally` (gotcha #10), and the post-extraction *work* is already delegated to named helpers — so a "full rewrite" would either be cosmetic or require restructuring the drain at real risk to the disconnect-finalization contract, with only the test suite as a net. The Validate-unit extraction + PR-1 are the safe, valuable slice; the remaining rewrite is left as a documented deliberate stopping point rather than open debt.
