...
- [x] 🟩 **Step 2.3: Paragraph alignment + indent/outdent** — TextAlign on paragraph/heading; sanitiser accepts `text-align` on `<p>/<h3>/<li>` + cells (pinned by `test_paragraph_alignment_survives`). Indent/outdent implemented (2026-06-23 follow-up) as a custom extension (`web/src/lib/notesIndent.ts`) — no first-party TipTap indent exists — rendering `margin-left` (em), sanitiser-allowed on `<p>/<h3>/<li>`. Pinned by `notesIndent.test.ts` + `test_paragraph_indent_survives`.
...
- [x] 🟩 **Step 3.4: Header-row toggle + per-cell alignment + column width** — all implemented (2026-06-23 follow-up). Header toggle via `toggleHeaderRow`. Per-cell alignment: `textAlign` added to the cell style model + `applyCellAlign` (Tier-2 Align group), persists as `text-align` on the cell. Column width: `Table.configure({ resizable: true })` → standard `<colgroup><col style="width">` round-tripped through the widened sanitiser (`<colgroup>`/`<col>` + `width`). Pinned by `cellFormatting.test.ts` (align across selection) + `test_notes_html_sanitize_css.py` (width/colgroup).
