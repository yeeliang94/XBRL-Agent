...
async def _invoke_sub_agent_once(
    ...
    prompt = (
        ...
        f"{batch_list}"
        f"{scope_line}"
        f"For each note, use the source and writing tools specified in your "
        f"system prompt to choose and fill the best-supported template row.\n\n"
        f"After your writes, call `submit_batch_coverage` "
        f"with its typed `entries` list accounting for EVERY note in your "
        f"batch except capture gaps already reported — one "
